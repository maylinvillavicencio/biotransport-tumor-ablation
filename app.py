...
class COMSOLVesselInterpolator:
    def __init__(self):
        ...
        self.temps[:, 0] = [37.01332988963907, 89.28820214853024, 91.82831014749000, 92.35752500000000, 92.51853700000000]
        self.temps[:, 1] = [37.01343745229815, 90.39493541249876, 92.68835103219796, 93.24920048749480, 93.42238392224195]
        self.temps[:, 2] = [37.01357406218045, 91.20003415821034, 93.83253548911045, 94.44218612048912, 94.62108754938538]
...
Utiliza regresores e interpoladores calibrados directamente con las soluciones numéricas de COMSOL Multiphysics.
""")

# --- CARGA DEL MODELO DESDE EL REPOSITORIO ---
if os.path.exists('best_model.pkl') and os.path.getsize('best_model.pkl') > 0:
    try:
        ...
        if isinstance(modelo_cargado, dict) and 'model_dano' in modelo_cargado:
            ...
            st.error("⚠️ El archivo 'best_model.pkl' detectado es antiguo o inválido.")
            st.stop()
    except Exception as e:
        st.error(f"Error al decodificar el archivo del modelo: {e}.")
        st.stop()
else:
    st.error("⚠️ No se encontró el archivo 'best_model.pkl'.")
    st.stop()

# --- DATOS COMSOL HISTÓRICOS ---
...
st.markdown("---")
st.subheader("🩸 Respuesta Térmica Exacta del Vaso Sanguíneo")
st.markdown(f"La línea morada representa la predicción del modelo subrogado para un vaso de **{diametro_input:.2f} mm**.")
...
ax3.set_title('Mapeo de Curvas Térmicas sin Oscilaciones Matematicas')
ax3.legend()
