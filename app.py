...
Utiliza regresores entrenados con las soluciones numéricas de la ecuación de Bioheat extraídas de COMSOL Multiphysics.
""")
...
# --- CARGA DEL MODELO PRE-ENTRENADO COLECTIVO (.PKL) ---
if os.path.exists('best_model.pkl') and os.path.getsize('best_model.pkl') > 0:
    try:
        paquete_modelos = joblib.load('best_model.pkl')
        modelo_dano = paquete_modelos['model_dano']
        modelo_vaso = paquete_modelos['model_vaso']
    except Exception as e:
        st.error(f"Error al decodificar el archivo del modelo: {e}. Por favor, vuelve a generar y subir 'best_model.pkl'.")
        st.stop()
else:
    st.error("⚠️ No se encontró el archivo 'best_model.pkl'. Asegúrate de haber ejecutado el nuevo 'training.py' localmente y haber subido el archivo resultante.")
    st.stop()
...
# NUEVO SLIDER INTERACTIVO PARA EL VASO SANGUÍNEO
escenario_input = st.sidebar.slider("Configuración de Flujo/Vaso (Escenario Continuo):", 1.0, 3.0, 1.5, step=0.1, help="1.0 = Escenario A, 2.0 = Escenario B, 3.0 = Escenario C. Permite interpolar estados intermedios.")

# Predicciones puntuales en vivo usando los modelos cargados
...
with col5:
    st.metric("Daño final @ 12 mm", f"{dano_12mm[-1]*100:.1f}%", delta=f"-{(dano_4mm[-1]-dano_12mm[-1])*100:.1f} pp vs 4mm")
with col6:
    st.metric("Daño final @ 20 mm", f"{dano_20mm[-1]*100:.1f}%", delta=f"-{(dano_4mm[-1]-dano_20mm[-1])*100:.1f} pp vs 4mm")
...
st.info(f"**Patrón identificado:** entre 4 mm y 20 mm, el daño final cae de {dano_4mm[-1]*100:.1f}% a {dano_20mm[-1]*100:.1f}% (una reducción de {reduccion_pct:.0f}%), y el pico de velocidad de daño (dD/dt) se retrasa y se aplana con la distancia.")

# --- SECCIÓN 3: AUTOMATIZACIÓN E INTERACTIVIDAD DEL VASO SANGUÍNEO ---
st.markdown("---")
st.subheader("🩸 Efecto del Vaso Sanguíneo: Enfriamiento Convectivo Dinámico")
st.markdown("""
¡Esta sección ha sido completamente **automatizada con IA**! El regresor subrogado no solo lee los escenarios fijos de COMSOL, sino que **interpola curvas continuas en tiempo real** según muevas los parámetros del flujo en la barra lateral.
""")

# Generar la predicción de la curva continua para el vaso basado en el slider dinámico
X_vaso_continuo = np.column_stack((tiempos_continuos, np.full_like(tiempos_continuos, escenario_input)))
pred_vaso_continua = modelo_vaso.predict(X_vaso_continuo)

fig3, ax3 = plt.subplots(figsize=(10, 4.5))
# Puntos dispersos originales (COMSOL histórico)
...
# Línea matemática generada por el Modelo de IA
ax3.plot(tiempos_continuos, pred_vaso_continua, color='purple', linestyle='--', linewidth=2.5, 
         label=f'Curva Predicha por IA (Escenario Configurado: {escenario_input:.2f})')

# Punto dinámico actual seleccionado por el usuario
ax3.plot(tiempo_input, prediccion_vaso_viva, marker='X', color='black', markersize=12, label='Punto Temporal en Vivo')

ax3.axhline(103, color='red', linestyle='--', alpha=0.5, label='Meseta ≈103 °C (Limbo Heat-Sink)')
...
st.warning("""
⚠️ **Nota metodológica del Modelo de IA:** Gracias a la interpolación del modelo matemático, ahora puedes evaluar estados hipotéticos intermedios entre los escenarios experimentales (A, B y C). La estabilidad de la curva confirma de forma automatizada que el fenómeno de **heat-sink** conserva su meseta robusta cerca de los 103 °C ante sutiles variaciones térmicas.
""")

st.markdown("""
---
### 🛠️ Diagrama de Flujo del Proceso Automatizado
`COMSOL (Daño + Vaso)` ➡️ `Pipeline de Doble Regresión Polinomial` ➡️ `Consolidación Integrada (.pkl)` ➡️ `Predicción Simultánea (Fracción & Temperatura)` ➡️ `Dashboard Interactivo Streamlit`
""")
